...
//Question: Group by department, then by salary range (LOW: <70k, MEDIUM: 70k-85k, HIGH: >85k)
...
//                .collect(Collectors.groupingBy(Employee::getDepartment,Collectors.groupingBy(employee -> employee.getSalary()>85000 ? "High" : employee.getSalary() < 70000 ? "Low" : "Medium")));
        Map<String, Map<String, List<String>>> list = employees.stream()
                        .collect(Collectors.groupingBy(Employee::getDepartment,Collectors.groupingBy(employee->employee.getSalary() > 85000 ? "High" :employee.getSalary()<70000 ? "Low" : "Medium",Collectors.mapping(Employee::getName,Collectors.toList()))));
